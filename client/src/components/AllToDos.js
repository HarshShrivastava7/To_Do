...
function AllToDos({ list, newToDo, setNewToDo }) {
  ...
  async function handleKeyPress(event) {
    if (event.key === "Enter") {
      try {
        const updateTodo = await fetch(`http://localhost:5000/todos/${updatedId}`, {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
          },
      ...
      <Panel header="All ToDo's" shaded>
        {list.map((value, index) => {
          return (
            <InputGroup style={styles}>
              <InputGroup.Addon>
                <Trash style = {{cursor : "pointer"}} onClick={() => deleteTodo(value.id)} />
              </InputGroup.Addon>
              <Input value={value.desc.toUpperCase()} disabled />
              <InputGroup.Addon>
                <UpdateRoundIcon style = {{cursor : "pointer"}} onClick={() => updateToDo(value)} />
              </InputGroup.Addon>
            </InputGroup>
          );
        })}
        ...
        <Modal size={size} open={open} onClose={handleClose}>
          ...
          <Modal.Footer>
            ...
            <Button onClick={() => handleKeyPress({key : "Enter"})} appearance="primary">
              Ok
            </Button>
          </Modal.Footer>
        </Modal>
